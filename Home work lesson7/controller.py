...
menu_items = ['Добавить запись', 'Отсортировать по ID',
              'Отсортировать по фамилии', 'Вывести список имен и фамилий', 'Выход']


def get_field(field_name: str, field_length: int):
    field = view.get_user_input(
        f'Введите {field_name} (макс. {field_length} символов)')
    while True:
